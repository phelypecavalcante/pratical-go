...
func main() {
	...
	for name, signature := range sigs {
		...
		g.Go(func() error {
			return sigWorker(ctx, path.Join(rootDir, name)+".bz2", signature, ch)
		})
		// go sigWorker(path.Join(rootDir, name)+".bz2", signature, ch)
	}
	...
	fmt.Printf("processed files in %v\n", duration)
	if !ok {
		os.Exit(1)
	}
}

func sigWorker(ctx context.Context, fileName, signature string, ch chan<- result) error {
	...
	if !r.match {
		return fmt.Errorf("error: %s mismatch", fileName)
	}
